...
WHATSAPP_SVG = ('<svg viewBox="0 0 32 32" width="30" height="30" fill="currentColor" aria-hidden="true">'
                '<path d="M16.04 3C9.4 3 4 8.38 4 15c0 2.24.62 4.42 1.8 6.32L4 29l7.86-1.76A12.04 12.04 0 0 0 16.04 27C22.66 27 28 21.62 28 15S22.66 3 16.04 3zm0 21.98c-1.9 0-3.76-.5-5.38-1.46l-.38-.22-4.66 1.04 1.06-4.54-.26-.4A9.9 9.9 0 0 1 6.1 15c0-5.46 4.46-9.9 9.94-9.9 5.48 0 9.9 4.44 9.9 9.9 0 5.46-4.42 9.98-9.9 9.98zm5.42-7.42c-.3-.14-1.76-.86-2.02-.96-.28-.1-.48-.14-.68.14-.2.3-.78.96-.96 1.16-.18.2-.36.22-.66.08-.3-.16-1.26-.46-2.4-1.48-.88-.8-1.48-1.76-1.66-2.06-.18-.3-.02-.46.14-.6.14-.14.3-.36.46-.54.14-.18.2-.3.3-.5.1-.2.04-.38-.02-.54-.08-.14-.68-1.64-.92-2.24-.24-.58-.5-.5-.68-.5h-.58c-.2 0-.52.08-.8.38-.26.3-1.04 1.02-1.04 2.5s1.08 2.9 1.22 3.1c.16.2 2.1 3.2 5.1 4.48.72.3 1.28.5 1.7.62.72.22 1.36.2 1.88.12.58-.08 1.76-.72 2-1.42.26-.7.26-1.3.18-1.42-.06-.12-.26-.2-.56-.34z"/></svg>')
...
def common(soup, prefix, page):
    ...
    if soup.body and not soup.select_one("a.fc-whatsapp"):
        wa = BeautifulSoup(
...
for html_path in sorted(ROOT.rglob("*.html")):
    ...
    for old, new in (("BL Renovations", NAME), ("+27 74 415 7569", PHONE),
